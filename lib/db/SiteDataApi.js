...
export async function resolvePostProps({
  ...
  post = props?.allPages?.find(p => {
    ...
    return (
      ...
      (typeof p.id === 'string' && slugCandidates.has(idToUuid(p.id))) ||
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  if (!pageRecordMap) {
    ...
    // Keep the site buildable when Notion is temporarily unavailable (for
    // example, when loadPageChunk is rate-limited or returns 403 on Vercel).
    // Returning an empty object removes required collection fields and makes
    // static path generation crash. EmptyData preserves the expected shape
    // and lets ISR retry the upstream request after deployment.
  ...
  if (pageIds?.length === 0) {
    collectionQuery = await patchCollectionQuery({
      pageRecordMap,
      collectionId,
      collectionView,
      viewIds,
    ...
    block = pageRecordMap.block || block
  ...
  if (pageIds?.length === 0) {
    console.error(
      '获取到的文章列表为空，请检查notion模板',
      {
        ...
        collectionViewKeys: Object.keys(collectionView || {})
      }
    )
  } else {
...
async function patchCollectionQuery({
  pageRecordMap,
  collectionId,
  collectionView,
  viewIds,
  ...
  const query = pageRecordMap.collection_query || {}
  const viewId = viewIds?.[BLOG.NOTION_INDEX || 0] || viewIds?.[0]
  ...
  try {
    ...
    console.warn('[Notion collection query] 获取失败', {
      collectionId,
      viewId,

...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  if (pageIds?.length === 0) {
    collectionQuery = await patchConfigCollectionQuery({
      pageRecordMap,
      collectionId,
      collectionView,
      viewIds,
...
async function patchConfigCollectionQuery({
  pageRecordMap,
  collectionId,
  collectionView,
  viewIds,
  ...
  try {
    ...
    Object.assign(pageRecordMap.block, collectionData?.recordMap?.block)
    Object.assign(pageRecordMap.collection, collectionData?.recordMap?.collection)
    Object.assign(
    ...
    console.warn('[Notion配置] queryCollection 获取失败', {
      collectionId,
      viewId,

...
export function GlobalContextProvider(props) {
  if (props.enableClerk) {
    return <AuthenticatedGlobalContextProvider {...props} />
  }

  return (
    <GlobalContextContent
      {...props}
      auth={{ isLoaded: true, isSignedIn: false, user: false }}
    />
  )
}

function AuthenticatedGlobalContextProvider(props) {
  const auth = useUser()
  return <GlobalContextContent {...props} auth={auth} />
}

function GlobalContextContent(props) {
  const {
    post,
    children,
    siteInfo,
